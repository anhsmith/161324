...
One thing that regression trees are particularly useful for are missing data.
...
The commands required to fit and make predictions from a model with `rpart` or `randomForest` are a little messy, and unfortunately `broom` doesn't sweep things tidily for tree based models[^broomstick].
