...
imm
```
Ok, this is getting somewhere. We have generally got the data we want there, but notice that the names aren't very useful. Also, we have all the ethnicity information, which is useful for some analyses, but we're going to keep things simple and just keep the first 5 columns. We can, of course, do that with `select()`. In this case, the names aren't currently in a nice format, so we'll rename the variables by way of their column numbers.

```{r}
names(imm)

imm_cleanish <- imm |> 
  select(Milestone = 1, DHB = 2, Eligible = 3, Immunised = 4, Proportion = 5)
...
```{r}
imm_cleaner <- imm_cleanish |> 
  filter(Milestone != "All Milstone Coverage",
         DHB != "National Total")
...
```{r}
imm_clean <- imm_cleaner |> 
  mutate(Eligible = as.numeric(Eligible))
...
```{r}
imm_clean |> 
  mutate(PropCheck = Immunised/Eligible) |> # Compute proportion directly
  mutate(Difference = abs(PropCheck - Proportion)) |> # Calculate difference
  filter(Difference > 1e-6) # Find any differences > 0 
```

After filtering out the columns with a non-zero difference, there are no columns. Neat - the proportion seems to be accurate. Now we can do some charting.
