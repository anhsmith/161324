...
Notice that we've named all the arguments (`data`, `mapping`, `x`, `y`) that we're passing to the functions (`ggplot`, `geom_point`, `aes`), but we don't have to. 

Instead of explicitly naming all the arguments like this:
...
... we can get the equivalent plot without naming the arguments, like this:
...
We can omit the names of the arguments, but only if they're given in the correct order that they appear in the function. That is, we can omit `data =` in the `ggplot()` function because `data` is the first argument for `ggplot()`. Likewise, for the `aes()` function, the first argument is `x` and the second argument is `y`. 
...
Of course, we can override the mapping if we like, e.g. to map `colour` to `species`.
...
1. Modify the above to use `geom_density()` instead of `geom_histogram()`.
...
The above code makes the process much clearer: "Take `roll`, filter so that `EthnicGroup` is 'International fee paying', then filter to 'Year 13' students, and arrange by descending number of students."
...
However, this will be wrong, as there are no entries where the number of students in a category is 0, yet we know that some schools will have no students of a particular ethnicity, gender, and level combination (e.g. single-gender schools).

```{r}
roll |> filter(Students == 0)
```

We can see this by summing the number of students by school and gender, and then taking a look at some schools:
