...
To deal with this, we need to scale the data down to something small. A typical way to do this is to scale down so that the mean of each predictor is 0 and the standard deviation is 1. This linear method of scaling, sometimes called 'normalisation', is achieved by taking each value of a variable $x$, subtracting the mean of $x$, and dividing by the standard deviation of $x$. In other words:

$$
x'_i = \frac{x_i-\bar{x}} {\text{sd}(x)}
$$
where $x'_i$ are the normalised values of the original variable $x_i$. You might also recall, from previous statistics courses, calculating standardised 'z-scores' from a variable--same deal. 

We scale linearly so we don't affect the shape of the data by subtracting the mean of each column and dividing by the standard deviation. Of course, make sure to scale the data the same way for both the training and test data.

One way to do scale the predictors is to compute the necessary scaling factors (mean and sd) on the training data, and then apply those scaling factors to both training and test data via the `scale` function. Using base R, this would be:
...
Then we apply to the training data by 

1. adding a 'rowid' (so we can pivot back wide again), 
2. joining to our scaling information, 
3. mutating to get the new value, and then 
4. pivoting back after removing the unused mean and sd columns. 

The same code can then be applied to `comp_test`
...
The scaling of the data above, whether we use the `scale` command directly or whether we stay within the `tidyverse` is rather cumbersome - and easy to mess up. For example, you might forget to apply the mean and sd from the training data to the test data, and instead accidentally recompute on the test data so the two scalings don't match.

Of course, there's a package that can help! The `recipe` package within the `tidymodels` framework can handle this.
...
We give to `recipe` the formula and data (so it knows the outcome variable and other potential predictors) and then add the `step_normalize` function to normalise all numeric predictors.

The recipe just describes what we want to do, but it doesn't actually do it! To perform the operation there are two more steps needed. We first must `prep`, which will estimate the parameters for scaling:
...
Then, we must `bake` our recipe using both our training and testing data. This will perform the transformations and return our datasets to us:
...
We can check the scaled datasets are the same as those we made manually by using `anti_join` - we should get no rows if they are the same:
...
4. Predict with our model the `bake`'d test data, and bind the predictions to the test data.
...
2. Try altering the above code to do more steps by setting the `epochs` parameter. By default, this is set to 100, so try 10,000 to run for 100 times longer. You should still see different metrics from run to run, but they shouldn't vary as much.
...
The goal is to build a predictive model for the `rejection` variable, the probability a customer rejects the bank. Some guidelines:
 ...
 - Choose the best model based on predictive performance (RMSE) on your test set.
 
Once you have found the best model, re-train it on all the data and use the model to obtain predictions for this dataset:

```{r}
bank_predict_me <- read_csv("https://www.massey.ac.nz/~jcmarsha/data/bank-predict-me.csv")
```

Notice that this second dataset does not have the known `rejection` variable - your goal is to provide the best predictions you can for these unknown cases.

Once you have your predictions for rejection, you can write it to a csv file (see `write_csv`) and email it to: j.c.marshall@massey.ac.nz
