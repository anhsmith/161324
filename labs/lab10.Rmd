...
2. Make a Principal Component Analysis plot of the data to see if there are any clear 'clusters' of points.
...
3. Produce a long-form dataset `party_long` by pivoting the vote columns into `Party` and `PartyVote` columns. Use this to produce boxplots of the `PartyVote` by `Party` to show the variation across the electorates.

4. Produce a long-form version of the `candidate` data, and combine both into a `all_long` dataset. Use this to do a scatterplot of the person vote versus the party vote, coloured by Party.
...
Let's now look into the clustering a little more, by using `cutree` to break things down into (say) 5 clusters:
...
Using the combined voting data `all`, perform hierarchical clustering and investigate which electorates tend to vote similarly when both party and candidate votes are considered.
