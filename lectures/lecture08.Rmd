...
GLMs are closely related to linear regression.

A critical difference is that the response (target) variable $y$ need not be distributed according to a normal distribution.
...
A solution is to use a type of sigmoidal function that you met when looking at neural networks.

We can model probability $p(\mathbf{x})$ via the **logistic** (or sigmoidal) function: $$p(\mathbf{x}) = \frac{1}{1 + e^{-\theta(\mathbf{x})}}$$ where $\theta(\mathbf{x})$ is a **linear predictor**, given by
...
Recall that $$p(\mathbf{x}_0) = \frac{1}{1 + e^{-\theta(\mathbf{x}_0)}}$$ 
where
...
Recall that this is equivalent to $\theta(\mathbf{x}_0) = \mathsf{logit}(p(\mathbf{x}_0)) \ge 0$.
...
-   For test case 0, $\hat \theta(\mathbf{x}_0) = -18.77$.
...
The probability of class $j$, $p_j(\mathbf{x}) = \mathsf{P}(j | \mathbf{x})$, is an increasing function of $\theta_j(\mathbf{x})$.
